    ...
    if displayed < 1:
        ...
        if show_goals and 'goalObservations' in episode_info:
            ...
            if len(goalObservations) > 0:
                ...
                display_response(goalObservations[0], display_surf, camera_outputs, print_observation=False,
                                 write_video=False)
...
def display_response(response, display_surf, camera_outputs, frame_id, print_observation=False, write_video=False):
    ...
    np_color = sensor_data["color"]["data"]
    np_depth = sensor_data["depth"]["data"]
    np_depth *= (255.0 / np_depth.max())  # naive rescaling for visualization
    np_depth = np_depth.astype(np.uint8)
    np_object_type = sensor_data["objectType"]["data"]
    np.save("{}_color.npy".format(frame_id), np_color)
    np.save("{}_depth.npy".format(frame_id), np_depth)
    np.save("{}_object_type.npy".format(frame_id), np_object_type)

    # img_color = Image.fromarray(np_color.reshape(480, 640, 4), "RGBA")
    img_depth = Image.fromarray(np_depth.reshape(480, 640))
    img_depth = img_depth.convert("L")
    # img_object_type = Image.fromarray(np_object_type.reshape(480, 640, 4), "RGBA")
    # img_color.save(os.path.join(save_location, "{}_color.png".format(frame_id)))
    img_depth.save(os.path.join(save_location, "{}_depth.png".format(frame_id)))
    # img_object_type.save(os.path.join(save_location, "{}_object_type.png".format(frame_id)))
...
def write_text(display_surf, text, position, font=None, fontname='monospace', fontsize=12, color=(255, 255, 224),
               align=None):
    """
    text -> string of text.
...
def interactive_loop(sim, args):
    ...
    if args.observations.get('forces'):
        ...
        camera_outputs['curr']['forces'] = {
            'area': pygame.Rect(next_start_x, display_height + font_spacing, plot_size, plot_size)}
        next_start_x += plot_size
    ...
    while sim.running:
        ...
        display_response(response, display_surf, camera_outputs['curr'], num_frames,
